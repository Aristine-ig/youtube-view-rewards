...
import { YouTubeTask } from "@/lib/types";
    ...
    >
      ...
      <div className="relative h-36 bg-secondary overflow-hidden">
        ...
        <div className="absolute bottom-2 left-3 z-20 flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-foreground/80">{task.channelName}</span>
        </div>
        <div className="absolute bottom-2 right-3 z-20 flex items-center gap-1 bg-background/60 backdrop-blur-sm rounded px-1.5 py-0.5">
          ...
          <span className="text-xs font-mono text-muted-foreground">{task.videoDuration}m</span>
        </div>
      </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border/50">
          ...
          <span className="stat-number text-sm text-reward">${task.totalReward.toFixed(2)}</span>
        </div>
      </div>
    </motion.div>
  );
